/**
 * ID Generator Service Implementation — starter prefixes.
 */

import type { DomainCode } from '@fedbounty/core/_shared/helpers';
import { DOMAIN_PREFIX_MAP, isValidDomainId } from '@fedbounty/core';
import { ulid } from 'ulid';
import type { IdGeneratorService } from '@fedbounty/services/_shared';

export function generateIdWithPrefix(prefix: string): string {
  if (!prefix || prefix.length !== 3 || !/^[a-z]{3}$/.test(prefix)) {
    throw new Error(
      `Invalid domain prefix: "${prefix}". Must be exactly 3 lowercase letters.`
    );
  }
  const id = `${prefix}_${ulid().toLowerCase()}`;
  if (!isValidDomainId(id)) {
    throw new Error(`Generated ID "${id}" failed validation.`);
  }
  return id;
}

export class DefaultIdGeneratorService implements IdGeneratorService {
  tntId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.tenant);
  }
  keyId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.apiKey);
  }
  idnId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.identity);
  }
  autId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.auth);
  }
  btyId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.bounty);
  }
  wrkId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.worker);
  }
  enrId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.enrollment);
  }
  subId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.submission);
  }
  vldId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.evaluation);
  }
  snpId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.snapshot);
  }
  rwdId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.payout);
  }
  escId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.escrow);
  }
  lngId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.lineage);
  }
  dspId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.dispute);
  }
  rptId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.report);
  }
  generateIdForDomain(domainCode: DomainCode): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP[domainCode]);
  }
}

let idGeneratorService: DefaultIdGeneratorService | null = null;

export function getIdGeneratorService(): DefaultIdGeneratorService {
  if (!idGeneratorService) {
    idGeneratorService = new DefaultIdGeneratorService();
  }
  return idGeneratorService;
}
