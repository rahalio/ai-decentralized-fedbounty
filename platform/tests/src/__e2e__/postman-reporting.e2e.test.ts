...
describe("Postman / reporting (1:1 generated)", () => {
  ...
  it("getBountyAuditReport", async () => {
    const url = sub("{{baseUrl}}/v1/reports/bounty-audit?bountyId={{bountyId}}");
  ...
  it("createPeriodReport", async () => {
    ...
    const res = await fetch(url, {
      ...
      body: sub("{\n  \"period\": \"\",\n  \"bountyId\": \"newman_bountyId\",\n  \"summary\": \"\"\n}"),
    });
    expect(res.status).toBe(201);
    const j = await res.json(); expect(j).toHaveProperty("data");
    if (j?.data?.id) vars['periodReportId'] = j.data.id;
  });

  it("getReport", async () => {
    const url = sub("{{baseUrl}}/v1/reports/{{reportId}}");
    const res = await fetch(url, {
      method: "GET",
      headers: vars.accessToken ? { Authorization: `Bearer ${vars.accessToken}` } : {},
    });
