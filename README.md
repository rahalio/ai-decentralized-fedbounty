...
OpenAPI-first DDD monorepo for **Fedbounty** — federated model-improvement bounty exchange with holdout-settled rewards.

Package scope: **`@fedbounty/*`**. Product specs: [PRODUCT.md](PRODUCT.md), [USER_STORIES.md](USER_STORIES.md), [WEBAPP.md](WEBAPP.md).
...
# Demo key: X-API-Key: fedbounty_demo_local_dev_key
```
...
```bash
docker compose up -d
TABLE_NAME=fedbounty-core-local AWS_ENDPOINT_URL=http://localhost:8000 node scripts/ensure-dynamo-table.mjs
```
